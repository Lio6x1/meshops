...
此目录归档个人账号、真实 HTTP 功能、场景控制、Go 测试、教程复制和容量实测。最新普通测试为 [168 PASS / 22 SKIP / 0 FAIL](unit-release.summary.json)，跳过不计作通过。独立容量共 [8 次记录](capacity/README.md)：一万实体 2000/s 与十万实体 500/s 对应档位通过；百万初始化及持久接收完成，但消费积压排空失败，不能称为百万端到端容量通过。完整真实依赖 race 已由 [CI 34744143433](https://github.com/Lio6x1/meshops/actions/runs/34744143433) 验收通过，精确实现提交为 `a19629ce0e7bf58388dea3355413be23279c2504`，见[执行元数据](ci-release.json)。后续收尾仅改文档和证据。
...
[教程最终验收](tutorial/README.md)完成 13 阶段构建、428 完整文件块复制与最后 5 个 Web 分步。原 18090 演示项目保留卷升级，所有运行服务健康检查、网页与 healthz 入口返回正常；管理员初始化状态为 false，需要使用者本机运行 `./scripts/account-admin.ps1 -Action Setup` 自行设置密码，未替用户创建固定密码。

## 清理结果

[可重建产物清理](cleanup.json)共删除 9,959,124,796 字节（约 9.28 GiB），包括旧可执行文件、前端依赖和 Go/静态检查缓存。[专用容器清理](docker-cleanup.json)移除了本轮三个隔离测试项目的停止容器与标签卷；原演示项目和其他服务保留。课程阶段、源码、工具和历史原始证据不当作冗余业务代码删除。

原始临时输出另保留于主工程 `.local/archives/accounts-scale-20260913/raw-verification.zip`（183 个文件，ZIP 完整性检查通过），见[本地归档摘要](private-archive.json)。此前主目录缓存中的历史诊断材料归入同目录的 `historical-context/`，固定工具保留原路径；未把历史来源丢弃或混入当前教程。
