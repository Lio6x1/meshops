...
更新：2026-09-13。介绍对应当前完整前后端工程；搜索、网页、模拟器和部署均已实现。以前“搜索未装配、没有提交 Git”的表述已移除。面试前快速复习见[速查卡](cheatsheet.md)。
...
| MySQL | 任务事实、审计、Outbox、投递记录、采样历史 | 同库事务覆盖数据库内事实，不覆盖 Kafka 和设备动作 |
...
| 哪些还没有验证？ | 真实硬件协议与动作、跨机故障和高可用、长时间大负载、完整用户体系和生产 TLS/密钥治理。 |
...
1. 按[启动手册](../run-fullstack.md)启动成品，登录操作员，展示六类实体与专属状态。
