...
复核基线：`641bf62`。本轮修复、独立源码复核、Windows 真实依赖集成、教材复制/逐步操作及本地运行恢复验收已完成，详见 [验收记录](acceptance.md)。最终 Git 提交、推送及对应 CI（含完整 Linux race）尚待完成。历史通过记录不能代替本轮验证。
...
| C4 订阅放大 | 已修复，Windows 集成通过 | 过滤后克隆、唯一实体版本批量核对、共享周期并绑定订阅生命周期；真实 Redis/gRPC 及注册/退出竞态回归已执行，最终 Linux race 待通过。 |
...
| T1 真实依赖未 race | 门禁已补，最终 race 待通过 | CI 依赖启动后完整 -race -tags integration，并检查关键测试及业务跳过。首轮 Linux 因三项基础设施/故障工具配置失败未通过；未检测到 DATA RACE 不能替代完整通过。 |
...
| T6 云 CI 未运行 | 基线已解决 | 641bf62 的 GitHub Actions 已成功；本轮仍需对最终提交复验。 |
| D1 搜索描述冲突 | 已修订 | 最终 Z10 已含搜索，Z09 有意保留加入搜索之前的能力；不将早期阶段描述套到最终工程。 |
| D2 验证清单过期 | 已更新当前状态 | 区分本轮本地通过、待提交/云 CI 与未纳入范围的能力；不把本地结果当远端结果。 |
| D3 代码块计数 | 已区分历史与当前 | 最新发布清单合计 865 个阶段文件记录、270 个完整代码块，Z10 最终 225 个文件；记录包含跨阶段重复，不能当独立源码文件数。日期报告保留当时数字。 |
| D4 本地链接 | 当前扫描通过，最终修改后再查 | 已有扫描 134 个 Markdown、1,759 个本地链接、0 失效；排错知识库另验 87 个相对链接。非完整 CommonMark 解析，非 Markdown fragment 与跨平台大小写仍有限制。 |
...
| H1 新工程未进 Git | 基线已解决 | 主工程已提交并推送；本轮使用独立工作树，最终整合、提交和核对范围。 |
...
| Linux 首轮全量 race | 三项基础设施/故障工具配置失败：容器内缺 Docker CLI，及 fault-Redis 精确地址保护。未检测到 DATA RACE，但该轮不通过；具备工具和标准隔离端口的最终提交 CI 仍须执行完整 race。 |
...
| 发布 | 最终范围核对、提交、推送及对应新提交的 GitHub Actions 均待完成；基线 CI 成功不能代替本轮。 |
