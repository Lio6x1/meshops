...
- [x] 在启动依赖后对 state/bus/edge/tasks/search 的真实路径执行 race，记录所需用例和异常退出。
...
- [x] 验证 Proto、静态检查、普通测试、真实依赖 race、完整答案复制、分步构建和 Git 检出指纹。
- [x] 将变更整合回主项目，确认本地记录和 UI 方案入口可打开，检查 Git 文件范围和远端 CI。
- [x] 输出四项交付与证据，明确剩余工程边界，不以测试通过保证不存在其他缺陷。

完成证据：本机完整运行/恢复/压力、教材复制与分步验证见 [验收记录](../../review/2026-09-12/acceptance.md)；修复提交 `31fda57` 的 [CI](https://github.com/Lio6x1/meshops/actions/runs/34680625332) 已成功。UI 已交付设计与模拟原型，浏览器视觉验证限制明确记录。

## 收尾补查 N10

- [x] 在旧故障逻辑上运行新容器身份门禁，实际失败。
- [x] 改为 start --wait，四项恢复、容器身份和 MySQL 命令参数比较通过；缺容器负例明确失败。
- [x] 同步 Z09 与最终答案，保存脱敏红绿证据和排错解释。
- [x] 追加变更完成十阶段复制构建、865 条指纹、提交整合、主目录启动和 a30432a 对应 CI 验收；证据见本轮验收记录。
