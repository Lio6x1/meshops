...
全程使用 `D:\job\golang\projects\meshops-course-lab`。Z01—Z03 按原讲义完成，之后依次执行下列 **28 个步骤**；不要把后续文件提前复制。每步结束版本由“上一步文件＋本页精确变化”确定，完整原文件均已提供。Z12-01 的前端尚处领域规则阶段，执行该步指定的 Node 测试；Z12-02 补齐组件后再构建网页。
...
| Z11 | [01](z11-01.md) → [02](z11-02.md) | [HTTP 网关与浏览器会话](../z11.md) |
| Z12 | [01](z12-01.md) → [02](z12-02.md) | [Vue 页面、状态规则与联调](../z12.md) |
| Z13 | [01](z13-01.md) | [完整 Docker 部署](../z13.md) |

Z10 是加入浏览器之前的固定阶段，完整来源是 `business-stages/z10`；根工程对应最后的 Z13。Z11/Z12 有 `web-stages` 固定答案。教材与学习代码分开，不把参考目录的凭证、运行数据、缓存或 node_modules 复制到自己的工程。

Z11—Z13 的首次安装、双终端开发、Docker 启动、个人账号初始化、浏览器演示与停止命令统一见 [完整启动手册](../../../../run-fullstack.md)。Z11 完成账号迁移和后端构建后，通过 `account-admin.ps1` 的 Setup 交互设置管理员密码；Z12 使用个人用户名和密码登录，管理员创建操作员，操作员首次登录必须改密。每步文件页用于确定“写哪些文件、运行哪些检查”，启动手册用于确定“当前模式怎样运行整个工程”。
