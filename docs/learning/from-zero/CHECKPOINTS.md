...
本页提供 Z01—Z10 的阶段代码与运行顺序。每个阶段都有完整源码，原项目框架无需复制进来。阶段代码验收与逐课详细讲义分开：Z07 是一个需要多课讲解的阶段，不能把一次复制130个文件当作一节入门课。

逐段讲解从 [新课程目录](lessons/README.md) 进入；手工路线在Z01—Z03之后，按Z04—Z10的 [23个小步骤](lessons/steps/README.md)复制精确增删改文件并验证。阶段答案复制脚本管理十个完整阶段，不管理这些课内小步骤，两条复制路线不要混用。
...
| Z10 | [根工程源码](../../..) | 一个 Search 服务、任务索引、Canal 同步、PIT 分页和维护重建；十个可执行程序 | 不包含全平台搜索、搜索容量结论或生产高可用 |
