...
演示里的六类实体可分别设为 0—5 个：打开“模拟演示”，输入数量后点“应用数量”，确认实际启用，再进入地图。默认每类 1 个，最多共 30 个；不需要修改 YAML 或重启容器。数量和模式会保留，缩减不删除历史及已有任务，完整操作见[模拟器与地图说明](simulation-map.md)。
...
两种方式的宿主 `18090` 会冲突。**同一时间选一种方式运行**；切换前按本文停止原来那套进程。Docker 演示固定使用 `meshops-demo` 项目名和独立卷，学习调试沿用课程的中间件项目；它们的数据和个人账号不是同一套。
...
设置初始管理员（PowerShell 7，密码在本机安全输入，不会显示）：

```powershell
./scripts/account-admin.ps1 -Action Setup
```

默认用户名 admin，可通过 -Username 指定。首次 Setup 创建账号，重复 Setup 只检查存在，不改变密码。密码由本机两次安全输入，经标准输入交给管理程序，不放入命令参数或凭证文件。管理员在账号管理页面创建操作员；操作员首次登录、被管理员重置后，必须先修改密码，成功后用新密码重新登录才能进入业务页。角色由账号决定，登录页没有管理员/操作员选择。密码采用 Argon2id 与随机盐，数据库不存明文。

浏览器打开 **`http://localhost:18090`**，输入管理员用户名和你自己设置的密码，之后可以使用个人操作员账号。也可使用 `http://127.0.0.1:18090`，但两个主机名的 cookie 独立，切换地址可能需要重新登录。
...
之后都用 `Up` 再启动。浏览器会话保存在网关内存中，重启网关后需要重新登录；账号及密码哈希保存在 MySQL 卷中，普通 Stop/Down 不会替换密码。注销、改密、重置及停用会撤销对应会话。
...
只有你确实想删除这套演示的所有数据库、消息、队列及账号，才执行下面的独立操作；它不是排错的默认第一步：
...
脚本限定在 `meshops-demo` 项目范围，`Reset` 会删除该项目的卷；下一次启动生成新数据；需再次 Setup 设置管理员。它不会替你备份旧任务。省略 `-ConfirmReset` 时脚本会拒绝执行。
...
新快照完整标记写入成功后，才重建 Canal 容器，并清理专属元数据目录中已经核实的 `meta.dat`、`h2.mv.db`、`h2.trace.db`、`h2.lock.db`。发现未知文件、子目录或符号链接会停止，让你先检查实际版本与配置。它不删除 MySQL 事实表、不删除 Kafka topic、不删除任何命名卷，也不改变账号密码。
...
凭证保存在同一专属卷内，但按 Linux 身份分文件授权：完整引导凭证只给 root；普通 Go 服务只读取公共可信 Registry 机器令牌、游标密钥与应用数据库密码；网关通过数据库校验个人账号，不再读取浏览器访问码；MySQL 单独读取 root 密码文件。普通服务仍共享演示 Registry 的机器身份，这不是生产环境中的完整服务级秘密隔离。再次初始化会核对派生凭证文件内容，不会静默覆盖不一致的文件。
...
8. **管理员入口**：使用管理员个人账号登录。运行状态显示分发器真实的三个采样指标。仅真实记录处于 `dlq` 时人工重试按钮可用；如果没有死信，不需要制造或修改数据库来凑演示。专用故障演练由验收步骤单独安排。
...
开发模式完成数据库迁移与构建后，运行 `./scripts/account-admin.ps1 -Mode Local -Action Setup` 设置该学习数据库的管理员。Docker 与本机数据库独立，账号也独立。旧 `.local/web-secrets.json` 不再用于当前登录。
...
| 登录后 401 | 地址主机名、个人用户名密码、会话是否重启/过期 | 重新登录，停用或重置后按管理员提供的方式恢复账号 |
| 403 | 网关日志、Origin/Host、当前角色与待改密状态 | 首次登录/重置后先改密；operator 不能重试死信、管理账号或读管理员状态；也可能是 CSRF 校验失败 |
